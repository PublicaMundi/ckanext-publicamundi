...
from ckanext.publicamundi.model.resource_identify import(
    ResourceIdentify, TaskNotReady, TaskFailed, IdentifyStatus, ResourceTypes)
...
class UserController(BaseController):
    ...
    def reject(self, resource_id, parent):
        ...
        res_identify_query = model.Session.query(ResourceIdentify).filter(
            ResourceIdentify.resource_id == resource_id).first()
    ...
    def render_ingestion_template(self, resource_id):
        ...
        c.task_result = json.loads(
            identification_helper.identify(resource_id)['result'])
        res_identify_obj = model.Session.query(ResourceIdentify).filter(
            ResourceIdentify.resource_id == resource_id).first()
