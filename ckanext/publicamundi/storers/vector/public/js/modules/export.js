ckan.module('export-module', function (jQuery, _) {
  return {
    ...
    initialize: function () {
	    var _base_url=this.options.base_url;
	    ...
	    init_export_module();
	    function init_export_module() {

	    $('#field-export_format').on('change', function() {
		val = this.value;

		found = (non_srs_formats.indexOf(val) > -1);
		...
		if(non_srs_formats.indexOf(export_format)<0){
		    if (is_valid_srs(projection)) {
			clear_previous_errors();
			do_request();
		    } else {

			raise_bad_srs(projection);
		    } 
		}
		else{
	...
	function is_valid_srs(projection) {
	...
	function raise_bad_srs(projection) {
	    $("#proj-error").show();
	...
	function do_request() {
	    ...
	    var export_url=_base_url+ "?format=" + $('#field-export_format').val() + "&projection=" + proj;
