...
def identify_resource(data, user_api_key):
    ...
    try:
        resource_tmp_folder, _file_path = _download_resource(
            resource, user_api_key)
        json_result = _identify(
            resource,
            user_api_key,
            resource_tmp_folder,
            _file_path)
...
def _identify(resource, user_api_key, resource_tmp_folder, file_path):
    ...
    gdal_driver, vector_file_path, prj_exists = _get_gdalDRV_filepath(
        resource, resource_tmp_folder, file_path)
...
def vectorstorer_upload(geoserver_cont, cont, data):
    ...
    try:
        resource_tmp_folder, filename = _download_resource(
            resource, user_api_key)
        _handle_resource(
            resource,
            db_conn_params,
...
def _download_resource(resource, user_api_key):
    '''Downloads the file declared in the resource url
    and saves it in a temp folder'''
    ...
    if resource['url_type']:
        ...
        _, params = cgi.parse_header(
            resource_download_request.headers.get(
                'Content-Disposition', ''))
...
def _handle_vector(
    ...
    if layer and layer.GetFeatureCount() > 0:
        ...
        wms_server, wms_layer = _publish_layer(
            geoserver_context, created_db_table_resource, srs_wkt)
        _add_wms_resource(
            context,
            layer_name,
...
def _add_db_table_resource(context, resource, geom_name, layer_name):
    ...
    created_db_table_resource = _api_resource_action(
        context,
        db_res_as_dict,
...
def _add_wms_resource(
    ...
    created_wms_resource = _api_resource_action(
        context,
        wms_res_as_dict,
        RESOURCE_CREATE_ACTION)
