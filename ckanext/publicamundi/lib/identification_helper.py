...
from ckanext.publicamundi.model.resource_identify import(
    ResourceIdentify, TaskNotReady, TaskFailed, IdentifyStatus)
...
def _get_identification_result(resource_id):
    ...
    res_identify_query = ckan.model.Session.query(ResourceIdentify).filter(
        ResourceIdentify.resource_id == resource_id)
    ...
    if res_identify:
        ...
        if ident_status == IdentifyStatus.NOT_PUBLISHED:
            status, result, resource_type = get_celery_identification_result(
                res_identify)
