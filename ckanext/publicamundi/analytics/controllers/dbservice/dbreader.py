from ckanext.publicamundi.analytics.controllers.configmanager import session
...
class DbReader:
    ...
    def read_used_coverages_totals(start_date, end_date):
        ...
        return HAParser.merge_info_list(used_coverages, HAUsedCoveragesInfo.coverage_name_property_key)
    ...
    def read_coverage_bands_totals(coverage_name, start_date, end_date):
        ...
        return HAParser.merge_info_list(coverage_bands, HACoverageBandsInfo.band_property_key)
    ...
    def read_coverage_access_by_date(coverage_name, start_date, end_date):
        coverage_access = session.query(HAUsedCoveragesInfo).filter(
            ...
            (HAUsedCoveragesInfo.date.between(start_date, end_date))).all()
        return HAParser.merge_info_list(coverage_access, HAUsedCoveragesInfo.date_key)

    @staticmethod
    def read_bbox_access_totals(start_date, end_date):
        bbox_access = session.query(HABboxAccessInfo).filter(HABboxAccessInfo.date.between(start_date, end_date)).all()
        return HAParser.merge_info_list(bbox_access, HABboxAccessInfo.bbox_key)

    @staticmethod
    def read_system_info():
        system_info = session.query(SystemInfo).filter(SystemInfo.key == SystemInfo.LATEST_DATE_KEY).one()
        return system_info
